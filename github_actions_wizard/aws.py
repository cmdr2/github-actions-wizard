...
def add_workflow_s3_cp_step(workflow, local_path, s3_path, acl="public-read"):
    step = {
        ...
        "run": f"aws s3 cp '{local_path}' 's3://{s3_path}' --acl {acl}",
        "shell": "bash",
    }
    workflow.add_job_step("deploy", step)
    return workflow
...
def add_workflow_s3_sync_step(workflow, local_path, s3_path):
    step = {
        ...
        "run": f"aws s3 sync '{local_path}' 's3://{s3_path}' --exclude '.git/*' --exclude '.github/*' --exclude 'LICENSE' --exclude 'README.md' --exclude '*.sh' --acl public-read",
        "shell": "bash",
    }
    workflow.add_job_step("deploy", step)
    return workflow
...
def add_workflow_lambda_deploy_step(workflow, function_name, zip_file):
    step = {
        ...
        "run": f"aws lambda update-function-code --function-name '{function_name}' --zip-file 'fileb://{zip_file}'",
        "shell": "bash",
    }
    workflow.add_job_step("deploy", step)
    return workflow
